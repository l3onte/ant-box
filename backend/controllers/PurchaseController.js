...
const getPurchases = async (req, res) => {
    try {
        ...
        const result = await PurchaseModel.getPurchases(
            id_tienda, 
            Number(page), 
            Number(limit), 
            search,
...
const postPurchase = async (req, res) => {
    try {
        const { id_tienda } = req.params;
        const result = await PurchaseModel.postPurchase(id_tienda, req.body);
...
const updatePurchase = async (req, res) => {
    try {
        ...
        const result = await PurchaseModel.updatePurchase(id_compra, req.body);
        res.status(200).json(result);
    } catch (error) {
...
const deletePurchase = async (req, res) => {
    try {
        const { id_compra } = req.params;
        const result = await PurchaseModel.deletePurchase(id_compra); // Cambia ShoppingModel por PurchaseModel
        res.status(200).json(result);
    } catch (error) {
